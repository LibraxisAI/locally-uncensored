...
export const RELEASE_NOTES: ReleaseNote[] = [
  ...
  // 3.0.0 selbst nicht enthielt.
  {
    ...
    lines: [
      ...
      'At a narrow window width, picking a model, opening the sampling sliders or opening the plugins menu used to leave the whole chat shifted sideways afterward, because the composer toolbar itself ran wider than the window and the browser scrolled the shared chat area to bring the clicked control into view. The toolbar now scrolls locally on its own when it does not fit, and those three popups measure the window instead of relying on a fixed spot, so the shared chat area itself never moves.',
    ],
    details: [
      {
        title: 'Fixes',
