...
export function PresetShelf({onSelect,resume,open,onOpenChange}:{onSelect:(p:CreatePreset)=>void;resume?:{title:string;onResume:()=>void}|null;open:boolean;onOpenChange:(o:boolean)=>void}) {
  ...
  return <aside aria-label="Presets" style={{width:open?260:52}} className="my-2 mr-2 flex shrink-0 flex-col overflow-hidden rounded-xl bg-lu-accent-soft ring-1 ring-lu-accent/20 transition-[width] duration-150">
    <div className={`flex shrink-0 items-center ${open?'gap-2 border-b border-lu-accent/15 px-3 py-2':'flex-col gap-1 py-2'}`}>
      <button aria-label={open?'Collapse presets':'Expand presets'} aria-expanded={open} onClick={()=>setOpen(!open)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md text-lu-accent hover:bg-lu-accent/10">{open?<PanelRightClose size={16}/>:<PanelRightOpen size={16}/>}</button>
      ...
      :<><div className="my-1 h-px w-6 bg-lu-accent/20"/>{resume&&<button aria-label={`Continue ${resume.title}`} onClick={resume.onResume} className="flex h-7 w-7 items-center justify-center rounded-md bg-lu-accent/15 text-lu-accent hover:bg-lu-accent/25"><Play size={11}/></button>}<button onClick={()=>setOpen(true)} className="py-3 t-micro font-medium tracking-wide text-lu-accent [writing-mode:vertical-rl]">Presets</button></>}
    </div>
    {open&&<div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-3">{CREATE_PRESETS.map(p=><button key={p.id} disabled={generating} onClick={()=>onSelect(p)} className="group block w-full overflow-hidden rounded-lg bg-black/10 text-left ring-1 ring-white/5 hover:ring-lu-accent/50 disabled:opacity-40"><img src={`/create-presets/${p.id}.webp?v=2`} alt={p.title} loading="lazy" className="aspect-video w-full object-cover"/><span className="block px-2.5 py-2"><span className="block t-control font-medium text-gray-200">{p.title}</span><span className="mt-1 block t-micro leading-4 text-gray-500">{p.summary}</span></span></button>)}</div>}
    {open&&<div className="shrink-0 border-t border-lu-accent/15 p-3"><Select size="sm" ariaLabel="Explore individual cloud models" placeholder="Individual cloud models…" value="" className="w-full" options={Object.entries(STUDIO_MODELS).map(([id,m])=>({value:id,label:m.label}))} onChange={id=>{const m=STUDIO_MODELS[id];if(m)onSelect({id,title:m.label,summary:'Configure your generation below.',category:'Video',adult:m.adult,accent:'',steps:[{model:id,kind:m.kind,op:'studio',title:m.label,role:roleForModel(id)??'image'}]})}}/></div>}
  </aside>
}
