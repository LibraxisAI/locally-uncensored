 ...
 */
function isComfyInstallComplete(comfyPath: string, running: boolean): boolean {
  if (running) return true
  return ['venv', '.venv'].some((v) => existsSync(join(comfyPath, v)))
}
...
export function registerComfyControlRoutes(routes: RouteMount, comfy: ComfyLauncher): void {
  ...
  routes.use('/local-api/find-comfyui', async (_req, res) => {
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' })
    if (!comfyPath) {
      res.end(JSON.stringify({ found: false, path: null, complete: false }))
      return
    }
    let running = false
    try { running = await isComfyRunning() } catch { /* ignore */ }
    res.end(JSON.stringify({ found: true, path: comfyPath, complete: isComfyInstallComplete(comfyPath, running) }))
  })
  ...
  routes.use('/local-api/detect-all-comfyui-installs', async (_req, res) => {
    ...
    let running = false
    try { running = await isComfyRunning() } catch { /* ignore */ }
    const installs = comfyPath
      ? [{
          path: comfyPath,
          complete: isComfyInstallComplete(comfyPath, running),
          has_embedded_python: existsSync(join(comfyPath, 'python_embeded')),
          source: 'scan',
        }]
      : []
    res.writeHead(200, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify(installs))
  })
}
